...
sPLSDAObject <- dSplsda(
  xYData = xYDataPaired, idsVector = testDataPaired$ids, groupVector = testDataPaired$label,
...
sPLSDAObject <- dSplsda(
  ...
  groupVector = testData$label, clusterVector = testDataDepeche$clusterVector, displayVector = subsetVector, createOutput = FALSE
)
