...
dViolins(testDataDepeche$clusterVector, testDataDepeche$sparsityMatrix[,1:2], inDataFrame = testData[, 2:3], createOutput = FALSE)
...
percentClusterVector <- DepecheR:::dScale(clusterVector, scale = c(0, 1), robustVarScale = FALSE, center = FALSE, 
...
oneClustAllVarList <- DepecheR:::dViolinsCoFunction2(inDataFocused[, 1], oneClustAllMu[1], allVarNames[1], clust = clustIndicesSpecific, 
    cols = clustColorsSpecific, clustNum = 3)
