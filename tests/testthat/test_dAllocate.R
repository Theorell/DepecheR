...
# Here, we are comparing if allocating the same data to the cluster centers genreated with depeche gives an ARI of 1, which it should
depRes <- depeche(x[[1]], center = FALSE)
allocRes <- dAllocate(x[[1]], depRes[[2]])
test_that("dAllocate expected output", {
  expect_true(DepecheR:::rand_index(depRes$clusterVector, allocRes, k = 100) == 1)
})
