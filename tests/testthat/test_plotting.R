...
dDensityPlot(xYData = testDataSNESubset, color = testColor, plotEachIdSeparately = TRUE, idsVector = testDataSubset$ids, 
    commonName = "sampling", createDirectory = FALSE, createPlot = FALSE)

# Alternative usage
dDensityPlot(xYData = testDataSNESubset, color = testColor, idsVector = testDataSubset$ids, commonName = "all samplings", 
    createDirectory = FALSE, createPlot = FALSE)

###################### dDensityPlotCoFunction
xYData <- DepecheR:::dScale(as.data.frame(testDataSNE$Y), as.data.frame(testDataSNE$Y), scale = c(0, 1), robustVarScale = FALSE, 
    center = FALSE)
cols <- colorRampPalette(c("black", "grey", "red"))(256)
densContour <- DepecheR:::dContours(as.data.frame(testDataSNE$Y))
DepecheR:::dDensityPlotCoFunction(xYData = xYData, cols = cols, name = "test", densContour = densContour, bandColor = "black", 
    dotSize = 1.5, title = FALSE, createPlot = FALSE)
