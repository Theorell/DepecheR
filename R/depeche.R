...
#' xDepecheObject <- depeche(x[,2:ncol(x)], dualDepecheSetup=data.frame(rep(1:2, each=6), 
#' colnames(x[,2:ncol(x)])), penalties=c(64, 128), sampleSizes=500, selectionSampleSize=500, 
#' maxIter=20, ids=x[,1])
#'
#' #Look at the result
#' str(xDepecheObject)
#' 
#' @export depeche
depeche <- function(inDataFrame, dualDepecheSetup, penalties=c(2^0, 2^0.5, 2^1, 2^1.5, 2^2, 2^2.5, 2^3, 2^3.5, 2^4, 2^4.5, 2^5), sampleSizes="default", selectionSampleSize="default", k=30, minARIImprovement=0.01, minARI=0.95, maxIter=100, ids, returnProcessedInData=FALSE, log2Off=FALSE){
  ...
  if(missing(dualDepecheSetup)==TRUE){
    if(missing(ids)==TRUE){
      depecheResult <- depecheCoFunction(inDataFrameScaled, firstClusterNumber=1, penalties=penalties, sampleSizes=sampleSizes, selectionSampleSize=selectionSampleSize, k=k, minARIImprovement=minARIImprovement, minARI=minARI, maxIter=maxIter)
    } else {
      depecheResult <- depecheCoFunction(inDataFrameScaled, firstClusterNumber=1, penalties=penalties, sampleSizes=sampleSizes, selectionSampleSize=selectionSampleSize, k=k, minARIImprovement=minARIImprovement, minARI=minARI, maxIter=maxIter, ids=ids)
    }
