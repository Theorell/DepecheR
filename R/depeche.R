...
#' 
#' @export depeche
depeche <- function(inDataFrame, dualDepecheSetup, penalties=c(2^0, 2^0.5, 2^1, 2^1.5, 2^2, 2^2.5, 2^3, 2^3.5, 2^4, 2^4.5, 2^5), sampleSize="default", selectionSampleSize="default", k=30, minARIImprovement=0.01, optimARI=0.95, maxIter=100, log2Off=FALSE, center="default", createOutput=TRUE){
  ...
  if(ncol(inDataFrame)<100){
    if(center=="mean"){
      ...
      inDataFrameScaleList <- dScale(inDataFrame, scale=FALSE, center="mean", returnCenter=TRUE)
      ...
      inDataFrameScaleList <- dScale(inDataFrame, scale=FALSE, center="peak", returnCenter=TRUE)
      inDataFramePreScaled <- inDataFrameScaleList[[1]]
      options(digits=14)
      logCenterSd[[2]] <- inDataFrameScaleList[[2]]
      options(digits=7)
  ...
  #logCenterSd[[3]] <- 1
  #inDataFrameScaled <- inDataFramePreScaled
  
  if(missing(dualDepecheSetup)==TRUE){
      depecheResult <- depecheCoFunction(inDataFrameScaled, firstClusterNumber=1, penalties=penalties, sampleSize=sampleSize, selectionSampleSize=selectionSampleSize, k=k, minARIImprovement=minARIImprovement, optimARI=optimARI, maxIter=maxIter, createOutput=createOutput, logCenterSd=logCenterSd)
  ...
  if(missing(dualDepecheSetup)==FALSE){
    ...
    depecheResultFirst <- depecheCoFunction(inDataFrameFirst, directoryName="Level_one_depeche", penalties=penaltyList[[1]], sampleSize=sampleSize, selectionSampleSize=selectionSampleSize, k=k, minARIImprovement=minARIImprovement, optimARI==optimARI, maxIter=maxIter, createDirectory=TRUE, createOutput=createOutput, logCenterSd=logCenterSd)
    ...
    depecheResultSecondList <- mapply(depecheCoFunction, inDataFrameSecondList, firstClusterNumberList, directoryNames, MoreArgs=list(penalties=penaltyList[[2]], sampleSize=sampleSize, selectionSampleSize=selectionSampleSize, k=k, minARIImprovement=minARIImprovement, optimARI=optimARI, maxIter=maxIter, createDirectory=TRUE, createOutput=createOutput, logCenterSd=logCenterSd), SIMPLIFY=FALSE)
