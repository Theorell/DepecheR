...
# @param initSampleSize The initial sample size. 
# @param sampleSizePowerIncrement With what value the sample size will increase. The value is taken to the power of two, i e, the standard value of 0.5 translates to a start value of initSampleSize*2^0, and the following value of initSampleSize*2^(0.5), and so on. 
...
# str(x_opt_clust)
# @export dOptAndClust
dOptAndClust <- function(inDataFrameScaled, ids, initSampleSize=2000, sampleSizePowerIncrement=0.5, maxSampleSize=100000, penalties=c(0,2,4,8,16,32,64,128), initCenters=30, maxIter=100, minImprovement=0.01){
  ...
  dOptObject <- dOpt(inDataFrameScaled, initSampleSize=initSampleSize, sampleSizePowerIncrement=sampleSizePowerIncrement, maxSampleSize=maxSampleSize, initCenters=initCenters, maxIter=maxIter, minImprovement=minImprovement, penaltyOptOnly=FALSE, penalties=penalties)
