removeEmptyVariablesAndAllocatePoints <- function(selectionDataSet, 
    ...
    # account
    clusterCentersNoEmptyVariables <- clusterCenters[, 
        which(colSums(clusterCenters) != 
            0)]
    if (length(which(colSums(clusterCenters) != 
        0) == 1)) {
    ...
    selectionDataSetNoEmptyVariables <- selectionDataSet[, 
        which(colSums(clusterCenters) != 
            0)]
    if (length(which(colSums(clusterCenters) != 
        0) == 1)) {
