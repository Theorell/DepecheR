dSplsdaPreCalculations <- function(clusterVector, idsVector, 
    groupVector, pairingVector, groupName1, groupName2) {
    ...
    # for each individual is created for both groups
    ...
    if (missing(pairingVector) == FALSE) {
        # Here, a comparable pairing vector pair is created if a
        # multilevel sPLS-DA should be performed.
        
        pairingVectorGroup1 <- as.character(pairingVector[groupVector == 
            unique(groupVector)[1]])
        pairingVectorGroup2 <- as.character(pairingVector[groupVector == 
            unique(groupVector)[2]])
    ...
    # A group vector is created with the same length as the
    ...
    # Here, the number of possible clusters to be saved in the
    # sPLS-DA is chosen. The number of tested clusters can be up
    ...
    for (i in 1:5) {
        if (i == 1) {
            ...
            if (testKeepAlternatives[i - 1] > 1) {
                testKeepAlternatives[i] <- round(testKeepAlternatives[i - 
                  1]/2)
            } else {
                break
            }
