dSplsdaPreCalculations <- function(clusterVector, 
    ...
    if (missing(pairingVector) == FALSE) {
        ...
        pairingAll <- c(pairingShortGroup1, 
            pairingShortGroup2)
    } else {
        pairingAll <- NULL
    }
    ...
    # tables
    groupId <- as.factor(c(rep(groupName1, 
        ncol(clusterFractionsForAllIds1)), 
    ...
    return(list(clusterFractionsForAllIds, 
        groupId, testKeepAlternatives, pairingAll))
}
