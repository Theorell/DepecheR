...
dOptPenalty <- function(inDataFrameScaled, k, maxIter, minARIImprovement, 
    sampleSize, penalties, makeGraph, disableWarnings = FALSE, 
    optimARI) {
    
    # The constant k is empirically identified by running a large
    # number of penalty values for a few datasets.
    ...
    # This command is reiterated the number of times that is
    # needed to reach a minimal improvement of the distance.
    iter <- 1
    std <- 1
    ...
    while (iterTimesChunkSize < 20 || (iterTimesChunkSize < maxIter && 
        ...
        # Before any further analyses are performed, any penalty that
        ...
        # Now, the new list is combined with the older, if there are
        # any
        if (iter == 1) {
            ...
            for (i in 1:length(optimListNonTrivial)) {
                ...
                # The same is done for all the matrices, to make the lists
                # the right length
                mockCenterMatrix <- matrix("NA", nrow = nrow(optimListFull[[1]][[5]][[1]][[1]]), 
                  ncol = ncol(optimListFull[[1]][[5]][[1]][[1]]))
                mockCenterMatrixList <- list(mockCenterMatrix, 
                  mockCenterMatrix, mockCenterMatrix, mockCenterMatrix)
        ...
        # Here, the average and standard deviation of the error of
        ...
        if (length(interestingPenalties) > 1) {
            ...
            meanMinus2StdMax <- meanOptimVector[maxPos] - (2 * 
                stdOptimVector[maxPos])
            
            # Now add the standard deviation of each of the non-minimal
            # values to the mean
            ...
            # Now, the distance between minMeanMinusSdAllNonMin and the
            # lowest value. If they overlap, the iteration has not made
            # it totally clear which point is optimal.
            distanceBetweenMaxAndSecond <- meanMinus2StdMax - 
                maxMeanPlus2StdAllNonMax
            
            # And now, the interesting positions are defined.  These are
            # the ones that either overlap with uncertainity with the
            # optimal solution, or that are very similar to it. In the
            # first iterations, the critera are more inclusive.
            if (iter == 1) {
                usedPositions <- which(realPenalties %in% interestingPenalties & 
                  (meanPlus2StdAll >= (meanMinus2StdMax - (2 * 
                    stdOptimVector[maxPos])) | meanOptimDf[, 
                    1] >= max(meanOptimDf[, 1]) - ((1 - optimARI) * 
                    4)))
            } else if (iter == 2) {
                usedPositions <- which(realPenalties %in% interestingPenalties & 
                  (meanPlus2StdAll >= (meanMinus2StdMax - stdOptimVector[maxPos]) | 
                    meanOptimDf[, 1] >= max(meanOptimDf[, 1]) - 
                      ((1 - optimARI) * 3)))
            } else {
                usedPositions <- which(realPenalties %in% interestingPenalties & 
                  (meanPlus2StdAll >= meanMinus2StdMax | meanOptimDf[, 
                    1] >= max(meanOptimDf[, 1]) - ((1 - optimARI) * 
                    2)))
            }
            
            # Here, all penalties and solutions that do not overlap with
            # the optimal solution are excluded from further
            # optimiations, to reduce calculation time.
            interestingPenalties <- realPenalties[usedPositions]
        } else {
            distanceBetweenMaxAndSecond <- 1
        }
        
        # Finally, another criterion on the gain of adding more rows
        # is included
        ...
        print(paste("Set ", iter, " with ", chunkSize, " iterations completed in ", 
            round(fullTime[3]), " seconds.", sep = ""))
        iter <- iter + 1
    }
    
    stopCluster(cl)
    
    print(paste("The optimization was iterated ", (iter - 1) * 
        chunkSize, " times.", sep = ""))
    ...
    # Here, the optimal penalty is selected. This is defined as
    # the lowest penalty that yields an ARI that is not lower
    # than 0.01 less than the best ARI.
    optimalPenalties <- roundPenalties[which(meanOptimDf[, 1] >= 
        max(meanOptimDf[, 1]) - (1 - optimARI))]
    
    # The best penalty is defined as the median penalty or the
    ...
    if (makeGraph == TRUE) {
        ...
        # Plot the data
        plot(log10(roundPenalties), meanOptimDf[, 1], pch = 16, 
            axes = FALSE, ylim = c(0, 1), xlab = "", ylab = "", 
            type = "b", col = "black", main = "Distance between bootstraps as a function of penalties values")
        ...
        mtext("Log10 of penalty values", side = 1, col = "black", 
            line = 2.5)
        
        # Add Legend
        legend("topleft", legend = "ARI (high is good)", text.col = "black", 
            pch = c(16, 15), col = "black")
        
        dev.off()
    }
    
    # Here, the list of solutions with the best penalty is
    # exported
