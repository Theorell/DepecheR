...
depecheAllData <- function(inDataFrameScaled, penalty, firstClusterNumber = 1, 
    k) {
    ...
    # Here, the best iteration is retrieved, if this is not one
    # with only 1 cluster
    ...
    # And here, the optimal results, given if an origo cluster
    # should be included or not, are retrieved further
    clusterVector <- returnLowest$i
    clusterCenters <- returnLowest$c
    
    # And here, the optimal results are made more dense by
    # removing empty rows and columns, etc.  Here, the numbers of
    # the removed clusters are removed as well, and only the
    # remaining clusters are retained. As the zero-cluster is not
    # included, the first cluster gets the denomination 1.
    ...
    reducedClusterCenters <- clusterCenters[which(rowSums(clusterCenters) != 
        0), which(colSums(clusterCenters) != 0)]
    ...
    # Make the row names the same as the cluster names in the
