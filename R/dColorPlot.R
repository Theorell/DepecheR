...
dColorPlot <-
  function(colorData,
           ...
           addLegend = FALSE,
           idsVector,
           drawColorPalette = FALSE,
           title = FALSE,
           createDirectory = TRUE,
           directoryName =
             "Variables displayed as color on SNE field",
           truncate = TRUE,
           bandColor = "black",
    ...
    if (class(colorData) != "numeric" &&
        class(colorData) != "integer"
        &&
        class(colorData) != "data.frame" &&
        class(colorData) != "character") {
      stop(
        "ColorData needs to be either a numeric vector, a character vector
    ...
    if (addLegend == TRUE && createPlot == TRUE) {
      ...
      colorIdsDataFrame <-
        data.frame(unique(colorData), unique(idsVector),
                   stringsAsFactors = FALSE)
      colorIdsDataFrame <-
