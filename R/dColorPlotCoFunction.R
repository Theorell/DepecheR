dColorPlotCoFunction <- function(colorVariable, name, xYData, title=FALSE, densContour, bandColor, dotSize, createPlot=TRUE){
  ...
  if(createPlot==TRUE){
    if(title==TRUE){
    plot(V2~V1, data=xYData, main=name, pch=20, cex=dotSize, cex.main=5, col=colorVariable, xlim=xlim, ylim=ylim, axes=FALSE, xaxs="i", yaxs="i")
  }
    if(title==FALSE){
      plot(V2~V1, data=xYData, main=NULL, pch=20, cex=dotSize, cex.main=5, col=colorVariable, xlim=xlim, ylim=ylim, axes=FALSE, xaxs="i", yaxs="i")
    }
    if(length(densContour)>1){
