...
#' }
#' @param densContour If density contours should be created for the plot(s) or not. Defaults to TRUE. If a density object, as generated by dContours, is included, this will be used for the internal scaling of the plot, allowing for density distribution checks of different subcompartments of the data with the same scaling. 
...
#'
#' @export dDensityPlot
dDensityPlot <- function(xYData, color="blue", commonName="All_density", plotEachIdSeparately=FALSE, idsVector, densContour=TRUE, title=FALSE, createDirectory=TRUE, directoryName=paste("Density plots for ", commonName, "s", sep=""), bandColor="black", dotSize=500/sqrt(nrow(xYData)), createPlot=TRUE){
  ...
  if(length(color)==1){
    ...
    if(plotEachIdSeparately==FALSE){
      dDensityPlotCoFunction(xYData=xYData, cols=cols, name=commonName, densContour=densContour, bandColor=bandColor, dotSize=dotSize, title=title, createPlot=createPlot)
    }   
    ...
    if(plotEachIdSeparately==TRUE){
      ...
      for (i in 1:length(uniqueIds)){
        dDensityPlotCoFunction(xYData=xYData[idsVector==uniqueIds[i],], cols=cols, name=paste(commonName, uniqueIds[i], "density", sep = " "), densContour=densContour, bandColor=bandColor, dotSize=dotSize, title=title, createPlot=createPlot)
      }
  ...
  if(length(color)>1){
    ...
    if(plotEachIdSeparately==FALSE && length(color)>1){
      dDensityPlotCoFunction(xYData=xYData, multipleColors=TRUE, colorList=colorList, name=commonName, densContour=densContour, bandColor=bandColor, dotSize=dotSize, title=title, createPlot=createPlot)
    ...
    if(plotEachIdSeparately==TRUE){
      ...
      for (i in 1:length(uniqueIds)){
        dDensityPlotCoFunction(xYData=xYData[idsVector==uniqueIds[i],], cols=colorList[[i]], name=paste(commonName, uniqueIds[i], "density", sep = " "), densContour=densContour, bandColor=bandColor, dotSize=dotSize, title=title, createPlot=createPlot)
      }
