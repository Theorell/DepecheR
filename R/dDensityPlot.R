...
dDensityPlot <-
  function(xYData,
           color = "blue",
           commonName = "All_density",
           plotEachIdSeparately = FALSE,
           idsVector,
           densContour = TRUE,
           title = FALSE,
           createDirectory = TRUE,
           directoryName = paste("Density plots for ",
                                 commonName, "s", sep = ""),
           bandColor = "black",
    ...
    if (length(color) > 1) {
      ...
      for (i in 1:length(colors)) {
        colorList[[i]] <-
          colorRampPalette(c("black", "grey", colors[i]))(256)
      }
      ...
      if (plotEachIdSeparately == FALSE && length(color) > 1) {
        ...
        colorIdsDataFrame <-
          data.frame(unique(color), unique(idsVector),
      ...
      if (plotEachIdSeparately == TRUE) {
        ...
        for (i in 1:length(uniqueIds)) {
          dDensityPlotCoFunction(
            xYData = xYData[idsVector == uniqueIds[i],],
            cols = colorList[[i]],
            name = paste(commonName, uniqueIds[i], "density",
                         sep = " "),
            densContour = densContour,
            bandColor = bandColor,
            dotSize = dotSize,
