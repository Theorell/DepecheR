...
# This function creates a dataframe with any chose size with
# bimodally distributed data in each column. The probability
# of the sizes of the respective populations in the bimodal
# distributions is stochastic.
#' @importFrom dplyr bind_rows
# @param samplings Number of individual probability samplings
# with identical standard deviation samplings, ie a simple
# simulated parallel to number of subjects.  @param dataCols
# Number of columns in the generated dataframe. Default is 5.
# @param observations Number of rows per donor in the
# generated dataframe. Default is 10000.  @return A dataframe
# with the specified numbers of columns and rows.  @examples
# #Generate a default size dataframe with bimodally
# distributed data x <- generateBimodalData() #Plot the first
# two variables in this dataframe plot(x[,2], x[,3]) @export
# generateBimodalData
generateBimodalData <- function(centers, prop = 0.3, dataCols = 5, 
    observations = 10000) {
    if (missing(centers)) {
        centers <- rbind(runif(dataCols) + 50, runif(dataCols) - 
            50)
    } else {
    ...
    ids <- c(rep(1, floor(prop * observations)), rep(2, observations - 
    ...
    pop2 <- tail(rands, n = observations - floor(prop * observations)) + 
        rep(centers[2, ], each = observations - floor(prop * 
            observations))
...
generateSparseData <- function(modeN = 5, dataCols = 100, observations = 10000) {
    ...
    for (i in 1:modeN) {
        ...
        samples[seq((1 + obsPerMode * (i - 1)), obsPerMode * 
            (i)), ] <- temp + rep(centers[i, ], each = obsPerMode)
        ids[seq((1 + obsPerMode * (i - 1)), obsPerMode * (i)), 
            ] <- i
    }
