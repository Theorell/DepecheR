...
#' @param sampleSizes This controls what fraction of the dataset that will be used to run the penalty optimization. A single value or a vector of values are accepted. Defaults to 10000. 
#' @param fMeasureSampleSize The size of the dataset used to find the optimal solution out of the many generated by the penalty optimization at each sample size. Defaults to 10000. 
...
#' @useDynLib DepecheR
#' @export dClust
dClust <- function(inDataFrameScaled, penalties=c(0,2,4,8,16,32,64,128), sampleSizes=10000, fMeasureSampleSize=10000, k=30, minCRIImprovement=0.01, maxCRI=0.05, maxIter=100, withOrigoClust, ids){
  ...
  if(length(penalties)>1 && sampleSizes[1]!=nrow(inDataFrameScaled)){
      dClustResult <- dOptSubset(inDataFrameScaled=inDataFrameScaled, sampleSizes=sampleSizes, k=k, maxIter=maxIter, maxCRI=maxCRI, minCRIImprovement=minCRIImprovement, penalties=penalties, fMeasureSampleSize=fMeasureSampleSize)
  }   
