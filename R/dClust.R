...
dClust <- function(inDataFrameScaled, dualClustSetup, clusterOnAllData="default", penalties=c(0,2,4,8,16,32,64,128), sampleSizes="default", selectionSampleSize="default", k=20, minCRIImprovement=0.01, maxCRI=0.05, maxIter=49, ids){
  ...
  if(missing(dualClustSetup)==FALSE){
    ...
    dClustResultSecondList <- mapply(dClustCoFunction, inDataFrameSecondList, firstClusterNumberList, MoreArgs=list(penalties=penaltyList[[2]], sampleSizes=sampleSizes, selectionSampleSize=selectionSampleSize, k=k, minCRIImprovement=minCRIImprovement, maxCRI=maxCRI, maxIter=maxIter), SIMPLIFY=FALSE)
