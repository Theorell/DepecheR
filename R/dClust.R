...
#' @param clusterOnAllData If all data should be included in the final clustering. "default" leads to inclusion of all data if the number of data points does not exceed 10000. TRUE and FALSE are also accepted. 
...
#' 
#' @export dClust
dClust <- function(inDataFrameScaled, dualClustSetup, clusterOnAllData="default", penalties=c(0,2,4,8,16,32,64,128), sampleSizes="default", selectionSampleSize="default", k=20, minCRIImprovement=0.01, maxCRI=0.05, maxIter=49, ids){

  if(missing(dualClustSetup)==TRUE){
    if(missing(ids)==TRUE){
      dClustResult <- dClustCoFunction(inDataFrameScaled, firstClusterNumber=1, penalties=penalties, sampleSizes=sampleSizes, selectionSampleSize=selectionSampleSize, k=k, minCRIImprovement=minCRIImprovement, maxCRI=maxCRI, maxIter=maxIter, clusterOnAllData=clusterOnAllData)
    } else {
      dClustResult <- dClustCoFunction(inDataFrameScaled, firstClusterNumber=1, penalties=penalties, sampleSizes=sampleSizes, selectionSampleSize=selectionSampleSize, k=k, minCRIImprovement=minCRIImprovement, maxCRI=maxCRI, maxIter=maxIter, ids=ids, clusterOnAllData=clusterOnAllData)
    }
    return(dClustResult)
  }
  ...
  if(missing(dualClustSetup)==FALSE){
    ...
    if(missing(ids)==TRUE){
    dClustResultFirst <- dClustCoFunction(inDataFrameFirst, penalties=penaltyList[[1]], sampleSizes=sampleSizes, selectionSampleSize=selectionSampleSize, k=k, minCRIImprovement=minCRIImprovement, maxCRI=maxCRI, maxIter=maxIter, clusterOnAllData=clusterOnAllData)
    } else {
      dClustResultFirst <- dClustCoFunction(inDataFrameFirst, penalties=penalties, sampleSizes=sampleSizes, selectionSampleSize=selectionSampleSize, k=k, minCRIImprovement=minCRIImprovement, maxCRI=maxCRI, maxIter=maxIter, ids=ids, clusterOnAllData=clusterOnAllData)
    }
    ...
    dClustResultTrainFuncList <- mapply(dClustCoFunction, inDataFrameSecondList, firstClusterNumberList, MoreArgs=list(penalties=penaltyList[[2]], sampleSizes=sampleSizes, selectionSampleSize=selectionSampleSize, k=k, minCRIImprovement=minCRIImprovement, maxCRI=maxCRI, maxIter=maxIter, clusterOnAllData=clusterOnAllData), SIMPLIFY=FALSE)
