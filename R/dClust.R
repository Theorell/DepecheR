...
#' 
#' @export dClust
dClust <- function(inDataFrameScaled, dualClustSetup, penalties=c(0,2,4,8,16,32,64,128), sampleSizes="default", selectionSampleSize="default", k=30, minCRIImprovement=0.01, maxCRI=0.05, maxIter=28, ids){
  ...
  if(missing(dualClustSetup)==FALSE){
    ...
    if(missing(ids)==TRUE){
    dClustResultFirst <- dClustCoFunction(inDataFrameFirst, penalties=penalties, sampleSizes=sampleSizes, selectionSampleSize=selectionSampleSize, k=k, minCRIImprovement=minCRIImprovement, maxCRI=maxCRI, maxIter=maxIter)
    } else {
    ...
    dClustResultTrainFuncList <- mapply(dClustCoFunction, inDataFrameSecondList, firstClusterNumberList, MoreArgs=list(penalties=penalties, sampleSizes=sampleSizes, selectionSampleSize=selectionSampleSize, k=k, minCRIImprovement=minCRIImprovement, maxCRI=maxCRI, maxIter=maxIter), SIMPLIFY=FALSE)
