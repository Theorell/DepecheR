dDensityPlotCoFunction <- function(xYData, multipleColors = FALSE, cols, 
                                   ...
                                   directoryName, createPlot = FALSE) {
    ...
    if (createDirectory == TRUE) {
        ...
        png(paste0(name, ".png"), width = 2500, height = 2500, units = "px", 
            bg = "transparent")
    }
    ...
    if (createPlot == TRUE) {
        if (title == TRUE) {
            plot(x2 ~ x1, data = df[order(df$dens), ], main = name, pch = 20, 
                 cex = dotSize, cex.main = 5, col = col, xlim = xlim, 
                 ylim = ylim, axes = FALSE, xaxs = "i", yaxs = "i")
        }
        if (title == FALSE) {
            plot(x2 ~ x1, data = df[order(df$dens), ], main = NULL, pch = 20,
                 cex = dotSize, col = col, xlim = xlim, ylim = ylim, 
