dDensityPlotCoFunction <- function(xYData, multipleColors = FALSE, 
    cols, colorList, name, densContour, bandColor, dotSize, title, 
    createPlot = TRUE) {
    ...
    if (multipleColors == FALSE) {
        ...
        ## Use densCols() output to get density at each point. The
        ## colors here are only supporting the coming order of the
        ## rows further down the script.
    ...
    if (multipleColors == TRUE) {
        # Divide the dataframe according to which color annotation
        # the event has
    ...
    # Plot it, reordering rows so that densest points are plotted
    # on top
    if (createPlot == TRUE) {
        if (title == TRUE) {
            plot(x2 ~ x1, data = df[order(df$dens), ], main = name, 
                pch = 20, cex = dotSize, cex.main = 5, col = col, 
                xlim = xlim, ylim = ylim, axes = FALSE, xaxs = "i", 
                yaxs = "i")
        }
        if (title == FALSE) {
            plot(x2 ~ x1, data = df[order(df$dens), ], main = NULL, 
                pch = 20, cex = dotSize, col = col, xlim = xlim, 
