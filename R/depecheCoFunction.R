...
depecheCoFunction <- function(inDataFrameScaled, firstClusterNumber = 1, 
    directoryName, penalties, sampleSize, selectionSampleSize, 
    k, minARIImprovement, optimARI, maxIter, newNumbers, createDirectory = FALSE, 
    ...
    # First, if the dataset is very, very big, a subset of it is
    # used to subset from. Otherwise the system memory needed to
    ...
    dOptPenaltyResult <- dOptPenalty(inDataFrameUsed, k = k, 
        maxIter = maxIter, sampleSize = sampleSize, penalties = penalties, 
        makeGraph = createOutput, minARIImprovement = minARIImprovement, 
        optimARI = optimARI)
    
    # Now over to creating the final solution Here, the
    # selectionDataSet is created
    ...
    # If the dataset is small, a new set of seven clusterings are
    # performed (on all the data or on a subsample, depending on
    # the sample size), and the maximum likelihood solution is
    ...
    if (nrow(inDataFrameUsed) < 10000) {
        ...
        # Now, the best run amongst all the runs with the largest
        # sample size is defined, by identifying the solution that
        # gives the highest mean f-measure for all the others.
        # First, all solutions are retrieved
        allSolutions <- dOptPenaltyResult[[3]]
        
        # Now, all clusterCenters are used to allocate the
        # selectionDataSet.
        ...
        # Here, the corrected Rand index with each allocationResult
        # as the first vector vector and all the others as individual
        # second vectors is identified
        ...
        # Now the solution being the most similar to all the others
        # is retrieved
        optimalClusterCenters <- unlist(allSolutions[[which(meanARIVector == 
            max(meanARIVector))[1]]])
        colnames(optimalClusterCenters) <- colnames(inDataFrameUsed)
        # Remove all rows and columns that do not contain any
        # information
        reducedClusterCenters <- optimalClusterCenters[which(rowSums(optimalClusterCenters) != 
            0), which(colSums(optimalClusterCenters) != 0)]
        
        # In the specific case that only one row is left, due to a
        # high penalty, the data needs to be converted back to a
        # matrix from a vector. The same is done if the number of
        # informative variables is just one.
        if (length(which(rowSums(optimalClusterCenters) != 0)) == 
            1) {
            reducedClusterCenters <- t(reducedClusterCenters)
        } else if (length(which(colSums(optimalClusterCenters) != 
            0) == 1)) {
        ...
        # And here, the optimal solution is created with the full
        # dataset
        clusterVectorEquidistant <- dAllocate(inDataFrameScaled, 
            reducedClusterCenters)
    }
    
    # Here, the optPenalty information is retrieved from the
    # optimal sample size run.
    optPenalty <- list(dOptPenaltyResult[[1]], dOptPenaltyResult[[2]])
    
    # Here, a cluster center matrix is created that relates its
    # values to the original, indata variables, which increases
    # the interpretability vastly.  First, the cluster centers
    # are multiplied by the standar deviation of the data
    ...
    # Here, a sparsity matrix is generated, as the sparsed out
    # variables will seem like they are situated in the middle,
    # when they are in fact pushed to the center of the variable
    # in question.
    sparsityMatrix <- reducedClusterCenters
    sparsityMatrix[sparsityMatrix != 0] <- 1
    
    # Here, a heatmap over the cluster centers is saved. Only
    # true if the number of clusters exceeds one.
    if (ncol(reducedClusterCenters) > 500) {
    ...
    } else if (nrow(reducedClusterCenters) > 1 && ncol(reducedClusterCenters) > 
        1) {
        ...
        for (i in 1:ncol(graphicClusterCenters)) {
            ...
            graphicClusterCenters[, i] <- dScale(reducedClusterCenters[, 
                i], scaledFocus, robustVarScale = FALSE, center = FALSE, 
                truncate = TRUE)
        }
