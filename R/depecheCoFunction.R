...
depecheCoFunction <-
  function(inDataFrameScaled,
    ...
    if (nrow(inDataFrameUsed) <= selectionSampleSize) {
      ...
      selectionDataSet <-
        inDataFrameUsed[sample(1:nrow(inDataFrameUsed), selectionSampleSize, replace = TRUE),]
    }
    ...
    if (nrow(inDataFrameUsed) < 10000) {
      ...
      allocationResultList <-
        foreach(i = 1:length(allSolutions)) %do% removeEmptyVariablesAndAllocatePoints(selectionDataSet = selectionDataSetMatrix, clusterCenters = allSolutions[[i]])
      ...
      registerDoSNOW(cl)
      meanARIList <-
        foreach(i = 1:length(allocationResultList)) %dopar% mean(sapply(
          allocationResultList,
          rand_index,
      ...
      optimalClusterCenters <-
        unlist(allSolutions[[which(meanARIVector == max(meanARIVector))[1]]])
      ...
      reducedClusterCenters <-
        optimalClusterCenters[which(rowSums(optimalClusterCenters) != 0), which(colSums(optimalClusterCenters) != 0)]
      ...
      rownames(reducedClusterCenters) <-
        rep(firstClusterNumber:(firstClusterNumber + (nrow(
          reducedClusterCenters
        )) - 1))
      
      # And here, the optimal solution is created with the full dataset
      clusterVectorEquidistant <-
        dAllocate(inDataFrameScaled, reducedClusterCenters)
    }
    ...
    if (is.logical(logCenterSd[[2]]) == FALSE) {
      ...
      for (i in 1:ncol(clusterCentersMultSd)) {
        focusMean <-
          logCenterSd[[2]][which(names(logCenterSd[[2]]) %in% colnames(clusterCentersMultSd)[i])]
        correctClusterCentersList[[i]] <-
          clusterCentersMultSd[, i] + focusMean
      }
      correctClusterCenters <-
        do.call("cbind", correctClusterCentersList)
      colnames(correctClusterCenters) <-
    ...
    if (ncol(reducedClusterCenters) > 500) {
      print(
        "as the number of variables in the result exceeds 500, it is not meaningful to produce a cluster center heatmap, so it is omitted"
      )
      ...
      for (i in 1:ncol(graphicClusterCenters)) {
        scaledFocus <-
          inDataFrameUsed[, colnames(inDataFrameUsed) == colnames(graphicClusterCenters)[i]]
        graphicClusterCenters[, i] <-
          dScale(
            reducedClusterCenters[, i],
            scaledFocus,
      ...
      if (createOutput == TRUE) {
        if (logCenterSd[1] == FALSE) {
          ...
          heatmap.2(
            as.matrix(graphicClusterCenters),
            Rowv = FALSE,
            Colv = FALSE,
            dendrogram = "none",
            scale = "none",
