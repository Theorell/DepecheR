...
groupProbPlot <- function(xYData, groupVector, dataTrans,
  ...
  if(length(rowsGroup1) != length(rowsGroup2)){
    ...
    if(min(groupLengths) < 10000){
      if(min(groupLengths) < 5000){
        stop("At least one of the groups has less than 5000 events, which
             will lead to a too low resolution for this analysis to be 
             meaningful. Use dResidualPlot instead.")
      }
      message(paste0("The groups are not matched in size and the smaller group
      has only ", min(groupLengths), " rows, leading to a reduction in neighbor
                     resolution to this level for both groups"))
    }
    rowsGroup1 <- rowsGroup1[sample(seq(1, length(rowsGroup1)), 
                                           min(groupLengths))]
    rowsGroup2 <- rowsGroup2[sample(seq(1, length(rowsGroup2)), 
                                           min(groupLengths))]
  }
  ...
  #robustVarScale method in the dScale function
    ...
    while(firstCluster<=length(allClusters)){
      ...
      #And here, all the clusters that are close to the nonIndata clusters are
      #selected
      if(length(clusterRange)>1){
        ...
        #have an effect in the cases where the number of events differ between
        #groups. 
        groupRowNumNeighListFocus <- 
          lapply(rowNumbersNeighListFocus, function(x) 
          return(x[x %in% groupNeighRowVec]))
        
        dataTransNeighListFocus <- lapply(groupRowNumNeighListFocus, function(x)
          return(dataTrans[x,]))
        
        dataReturnListFocus <- lapply(groupRowNumNeighListFocus, function(x)
          return(groupValVec[x]))
        
        i <- 1
        resultFocus <- 
          foreach(i = seq_along(dataTransListFocus),
                  .packages="DepecheR") %dopar% microClust(
        ...
        rowNumbersNeighFocus <- 
            unlist(rowNumbersList[allClusters %in% distCenters[clusterRange,]])
        #This list is now focused on the group neighbors, which will only
        #have an effect in the cases where the number of events differ between
        #groups. 
        ...
        resultList[[x]] <- microClust(dataCenter=dataTransFocus, 
      ...
      print(paste0("Clusters ", clusterRange[1], " to ", clusterRange[length(clusterRange)], " smoothed in ", timeAfter, " ", attributes(timeAfter)$units, ". Now, ", length(allClusters)-clusterRange[length(clusterRange)], " clusters are left."))
