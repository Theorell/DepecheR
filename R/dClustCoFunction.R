...
dClustCoFunction <- function(inDataFrameScaled, firstClusterNumber=1, penalties=c(0,2,4,8,16,32,64,128), sampleSizes="default", fMeasureSampleSize="default", k=30, minCRIImprovement=0.01, maxCRI=0.05, maxIter=100, withOrigoClust="no", ids){
  ...
  dClustResult <- dOptSubset(inDataFrameScaled=inDataFrameUsed, firstClusterNumber=firstClusterNumber, sampleSizes=sampleSizes, k=k, maxIter=maxIter, maxCRI=maxCRI, minCRIImprovement=minCRIImprovement, penalties=penalties, fMeasureSampleSize=fMeasureSampleSize, withOrigoClust=withOrigoClust)
