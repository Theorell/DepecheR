...
![Adjusted Rand Index as a function of penalty values. Each value is based on at least 7 clustering pairs (DepecheR standard in a 8 core machine).](https://github.com/jtheorell/DepecheRVinjettePics/raw/master/Distance_as_a_function_of_penalty_values.jpg){ width=90% }
...
![Cluster center heatmap. Marker names on the x-axis, cluster numbers on the y-axis.](https://github.com/jtheorell/DepecheRVinjettePics/raw/master/Cluster_centers.jpg){ width=90% }
...
![Cluster distribution over the tSNE field.](https://github.com/jtheorell/DepecheRVinjettePics/raw/master/Fig_3_DepecheR_markdown.jpg){ width=90% }
...
png picture, this is not the standard, for publication reasons. 
```{r}
dColorPlot(colorData = testData[2:15], xYData = testDataSNE$Y, title=TRUE)

```
![Example of a few markers distribution over the tSNE field.](https://github.com/jtheorell/DepecheRVinjettePics/raw/master/Fig_4_DepecheR_markdown.png){ width=90% }
...
```
![Distribution of events in the different groups.](https://github.com/jtheorell/DepecheRVinjettePics/raw/master/Fig_5_DepecheR_markdown.jpg){ width=90% }
...
through the multi-dimensional datacloud created by the individual donor 
frequencies in each cluster that most optimally separates the groups. It then 
internally checks how well the groups are separated along this vector, and plots
the clusters that contribute to this separation with colors relative to how well
the groups are separated. As this method is a sparse version of the method, it,
like depeche, only identifies clusters that contribute robustly to separating
the clusters, and has its internal tuning algorithm to define how the penalty 
term should be set. For both methods, there are paired alternatives. In this 
case, however, the data is not paired, and thus, the normal methods will be 
...
![Residual, Wilcoxon, sPLS-DA plots.](https://github.com/jtheorell/DepecheRVinjettePics/raw/master/Fig_6_DepecheR_markdown.jpg){ width=90% }
...
![Example of the distribution of events in the four markers defining cluster 3, one of the significantly different clusters.](https://github.com/jtheorell/DepecheRVinjettePics/raw/master/Fig_7_DepecheR_markdown.png){ width=90% }
